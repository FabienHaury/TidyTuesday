...
#' Prépare la sauvegarde d'un graphique avec ragg::agg_png
#'
#' Cette fonction ouvre un device PNG via ragg::agg_png dans le dossier daté
#' correspondant à la date fournie. Elle ne ferme pas le device et ne dessine
#' pas le graphique : cela doit être fait en dehors.
#'
#' @param date_val Date ou chaîne "AAAA-MM-JJ" pour créer le dossier
#' @param nom_fichier Nom du fichier (avec extension .png)
#' @param width Largeur (en pouces, par défaut 8)
#' @param height Hauteur (en pouces, par défaut 8)
#' @param res Résolution en dpi (par défaut 300)
#' @param forcer Si TRUE, écrase le fichier existant
#' @return Chemin complet du fichier PNG créé
#'
#' @examples
#' # Exemple d'utilisation :
#' # chemin <- ouvrir_device_ragg(Sys.Date(), "graphique.png")
#' # ggplot2::autoplot(mpg)
#' # grDevices::dev.off()
ouvrir_device_ragg <- function(
  date_val,
  nom_fichier,
  width = 8,
  height = 8,
  ...
  if (missing(nom_fichier) || nom_fichier == "") {
    stop("Erreur : veuillez fournir un nom de fichier (.png) valide.")
  }
  ...
  message("Device ragg::agg_png ouvert pour : ", chemin_fichier)
  return(invisible(chemin_fichier))
}
